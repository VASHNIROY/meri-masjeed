...
import ErrorHandler from "../utils/ErrorHandler.js";
import { pool } from "../utils/db.js"; // Assuming `pool` is exported from db.js
import CatchAsyncError from "./catchAsyncError.js";
import jwt from "jsonwebtoken";

export const isAuthenticatedSuperAdmin = CatchAsyncError(async (req, res, next) => {
  const authorizationHeader = req.headers.authorization;
  if (!authorizationHeader) {
    return next(new ErrorHandler("Please provide an access token", 400));
  }
  ...
  try {
    ...
    pool.getConnection((err, connection) => {
      ...
      // Query the user data from your MySQL database using the acquired connection
      connection.query("SELECT * FROM superadmin WHERE email = ?", [decoded.email], (error, results) => {
...
export const isAuthenticatedAdmin = CatchAsyncError(async (req, res, next) => {
  ...
  try {
    ...
    pool.getConnection((err, connection) => {
      ...
      // Query the user data from your MySQL database using the acquired connection
      connection.query("SELECT * FROM admin WHERE email = ?", [decoded.email], (error, results) => {
        connection.release(); // Release the connection back to the pool
