...
export const addMasjeedToFavourite = CatchAsyncError(async (req, res, next) => {
  try {
    ...
    const checkExistenceQuery =
      "SELECT * FROM superadminfavouritemasjeeds WHERE favouritemasjeddid = ?";
    pool.getConnection((err, connection) => {
      if (err) {
        // Handle connection error
        console.error("Error acquiring connection:", err);
      ...
      connection.query(checkExistenceQuery, [id], (selectErr, results) => {
        ...
        const insertMasjeedIdQuery =
          "INSERT INTO superadminfavouritemasjeeds(favouritemasjeddid) VALUES (?)";
...
export const getFavouriteMasjeeds = CatchAsyncError(async (req, res, next) => {
  try {
    const favouriteMasjeedsQuery = "SELECT * FROM superadminfavouritemasjeeds";
    console.log("entered");
    ...
    pool.getConnection((err, connection) => {
      ...
      connection.query(favouriteMasjeedsQuery, async (selectErr, results) => {
        if (selectErr) {
        ...
        for (const favouritemasjeddid of results) {
        ...
        // Release the connection back to the pool before sending the response
        connection.release();
    ...
    try {
      ...
      pool.getConnection((err, connection) => {
        ...
        connection.query(checkExistenceQuery, [id], (selectErr, results) => {
          ...
          const deleteQuery =
            "DELETE FROM superadminfavouritemasjeeds WHERE favouritemasjeddid = ?";
