...
function SalahTimingTable() {
  ...
  const handleSaveClick = (id) => async () => {
    ...
    try {
      ...
      setRows((prevRows) => prevRows.map((row) => (row.id === id ? updatedRow.data : row)));
  ...
  const processRowUpdate = async (newRow) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setRows((prevRows) =>
          prevRows.map((row) => (row.id === updatedRowWithId.data.id ? updatedRowWithId.data : row))
        );
  ...
  const handleAction1 = async (id) => {
    try {
      ...
      const {day, fajr, shouruq, dhuhr, asr, maghrib, isha,month } = editedRow;
      ...
      const response = await fetch(`http://localhost:3009/api/v1/addhrtorow`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
           Authorization: `Bearer ${token}`,
  ...
  const handleAction2=async()=>{
    try {
      ...
      // Make the API call to update the row
      const response = await fetch(`http://localhost:3009/api/v1/substracthrtorow`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
           Authorization: `Bearer ${token}`,
  ...
  useEffect(() => {
    ...
    const filteredData = rows && rows.filter(row => row.month === parseInt(selectedMonth));
